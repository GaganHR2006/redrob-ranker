...
def show_cohort_analysis(candidates_with_scores: list):
    ...
    for rank_idx, (c, s) in enumerate(sorted_candidates[:50]):
        ...
        rows.append({
            ...
            "Gem": "✅" if s.get("hidden_gem") else "❌",
            "Deception": {"low": "🟢 Low", "medium": "🟡 Medium", "high": "🔴 High"}.get(s.get("deception_risk"), s.get("deception_risk", "")),
        })
...
def main():
    ...
    if uploaded:
        ...
        if sample_path.exists():
            ...
            st.error("sample_candidates.json not found. Please upload a JSON file via the uploader above.")
            return
    ...
    with tab_explorer:
        ...
        for rank_idx, (c, s) in enumerate(candidates_with_scores):
            ...
            all_export_rows.append({
                ...
                "Deception Risk": s.get("deception_risk"),
        ...
        with col_dl:
            st.download_button(
                ...
                data=df_all_export.to_csv(index=False),
        ...
        if len(candidates_data) <= 20:
            ...
            for rank_idx, (candidate, scores) in enumerate(candidates_with_scores):
                ...
                with st.container(border=True):
                    ...
                    show_details = col_chk.checkbox("Expand", key=f"show_sm_{cand_id}_{rank}")
                    if show_details:
            ...
            for rank_idx, (candidate, scores) in enumerate(candidates_with_scores[:5]):
                ...
                with st.container(border=True):
                    col_lbl, col_chk = st.columns([8, 2])
                    col_lbl.markdown(f"**{label}**")
                    show_details = col_chk.checkbox("Expand", key=f"show_lg_{cand_id}_{rank}")
                    if show_details:
                        show_candidate_evaluation(candidate, scores, rank)
